...
namespace 부대교전설정
{
    //=======================================================================================
    // 설정변수
    const int  부대목표변경_세력대상 = 1;     // 0: 컴퓨터AI 와 플레이어_위임군단 모두,  1: 플레이어_위임군단만, 2: 컴퓨터AI만
	...
	class Main
	{
        ...
        {            
            ...
            if (!check_force_option(force)) 
            {
                pk::info(pk::format("{} 해당세력이 아님으로 종료", pk::decode(pk::get_name(force))));
                return;
            }
            // 부대 교전 오더 처리
        ...
        //---------------------------------------------------------------------------
        // 세력 확인 함수 (force용)
        bool check_force_option(pk::force@ force)
        {
            if (!pk::is_alive(force)) return false;
            if (!pk::is_normal_force(force)) return false;
            
            // 0: 컴퓨터AI 와 플레이어_위임군단 모두 (플레이어 직접 조작 제외)
            if (부대목표변경_세력대상 == 0)            
                if (pk::is_player_controlled(force)) return true;
                
            // 1: 플레이어_위임군단만
            if (부대목표변경_세력대상 == 1)            
                if (!(force.is_player() and !pk::is_player_controlled(force))) return true;
                         
            // 2: 컴퓨터AI만 (플레이어 세력 모두 제외)
            if (부대목표변경_세력대상 == 2)            
                if (!force.is_player()) return true;
