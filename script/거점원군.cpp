...
namespace 거점원군
{
    ...
    const int  거점원군_최대전투병력 = 100000;   // 전투부대 출진 시 최대병력 기준
    const int  거점원군_최소출진병력 = 5000;   // 출진할 부대 최소병력 기준, 여유 병력이 최소병력 기준 미달 시 출진 안함
    const int  거점원군_도시출진거리 = 3;      // 도시인 경우 거리 만족 거점 있을 시 원군 출병 (1 ~ 3 사이), 관문/항구는 무조건 거리 1 이내만 검색
    const int  거점원군_출진거점무장수 =1;     // 원군출진할 거점의 잔류무장수, 기준수 이상이고 미행동 있을 경우 출진가능 (적 거점 주변에 있을 경우 잔류무장수 비례증가)
    ...
    const int  거점원군_교전거점무장수 = 100;     // 부대종류모드2 적용 시 : 교전거점의 무장수 기준 이상일 경우 무장충분으로 판단하여 수송부대 출병
