...
namespace 거점방어
{
    ...
    const int  거점방어_최대출진부대수 = 50;     // 한 거점에서 연속으로 방어출진 가능한 부대수 (1~50)
    const int  거점방어_거점최소병력 = 3000;     // 해당 관문/항구에서 출진 후 남길 최소병력
        ...
        void func_base_defence(pk::force@ force)
        {
            ...
            for (int i = 건물_관문시작; i < 건물_항구끝; i++)
            {
                ...
                if (base.get_force_id() == force_id and enemy_unit_around(base))
                {
                    ...
                    int max_count = pk::max(1, pk::min(50, 거점방어_최대출진부대수));
