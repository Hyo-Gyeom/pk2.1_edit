...
namespace 거점원군
{
    ...
    const int  거점원군_적군검색범위 = 10;    // 교전 거점에서부터 적부대를 검색하는 거리 (관문/항구는 최대 6으로 제한됨)
    
    const int  거점원군_거점최소병력 = 10000;  // 적거점 비인접한 후방거점, 기준병력 이상 보유 시 초과 병력만큼 원군 수송
    const int  거점원군_경계병력단위 = 5000;   // 적거점과 인접한 경우, 경계병력 단위로 적병력에 비례하여 거점최소병력 증가
    
    const int  거점원군_최대전투병력 = 20000;   // 전투부대 출진 시 최대병력 기준
    const int  거점원군_최소출진병력 = 3000;   // 출진할 부대 최소병력 기준, 여유 병력이 최소병력 기준 미달 시 출진 안함
    const int  거점원군_도시출진거리 = 3;      // 도시인 경우 거리 만족 거점 있을 시 원군 출병 (1 ~ 3 사이), 관문/항구는 무조건 거리 1 이내만 검색
    const int  거점원군_출진거점무장수 = 3;     // 원군출진할 거점의 잔류무장수, 기준수 이상이고 미행동 있을 경우 출진가능 (적 거점 주변에 있을 경우 잔류무장수 비례증가)
    ...
    const int  거점원군_전투부대수 = 50;      // 한 턴에 출진가능한 최대 부대수 (단, 거점간 거리에 따라 한 거점에서 출진가능 부대수 거리당 1씩 차감, 최소 1부대 가능)
    ...
    const int  거점공략_전투부대수  = 50;     // 공략지원군 전투부대 출진 부대수 (최소1 ~ 최대3)
	...
	class Main
	{
		Main()
		{
			// 111 이벤트로 변경하여 턴 시작 시 먼저 실행되도록 설정 (priority 0으로 가장 먼저 실행)
			pk::bind(111, 0, pk::trigger111_t(onTurnStart));
		}
        
		// 턴 시작 시 거점원군 실행 (부대교전설정보다 먼저 실행)
                    ...
                    and !needReinforce(src) and !list_reinforce_base.contains(src) and !list_attack_base.contains(src))
                {
                    ...
                    if ( (0 <= base_dist and base_dist <= max_distance) or city_dist == 1)
                    {
                            ...
                            and src_troops > ref_troops and weapon_amount >= ref_troops and src_food >= int(1.5f * ref_troops))
                        {
                            ...
                            else if (reinforce_mode)  // 적세력, 공성병기 보유유무 추가 확인
                            {
                                best_src = src_id;
                                src_list.add(src);  // 출진가능 거점리스트
                            }
        ...
        int PushCombatUnit(pk::building@ src_base, pk::building@ dst_base, bool do_cmd = true)
        {
            ...
            if (디버그모드)
            {
                string src_name  = pk::decode(pk::get_name(src_base));
                string dst_name  = pk::decode(pk::get_name(dst_base));
                pk::info(pk::format("[PushCombatUnit] 시작 {}→{}", src_name, dst_name));
            }
            ...
            // 육상 무기 선택
            get_ground_weapon(src_base, reinforce_troops, ground_weapon_id, unit_troops);
            if (ground_weapon_id == 0) return -1;    // 병기 부족
            if (디버그모드)
            {
                int stock_ground = pk::get_weapon_amount(src_base, ground_weapon_id);
                pk::info(pk::format("[CHK-WPN-G] ground_id={} stock={} need={}", ground_weapon_id, stock_ground, pk::max(1, unit_troops)));
            }
            ...
            if (디버그모드)
            {
                int stock_water  = pk::get_weapon_amount(src_base, water_weapon_id);
                pk::info(pk::format("[CHK-WPN-W] water_id={} stock={} need={}", water_weapon_id, stock_water, ((water_weapon_id == 병기_주가)? 0 : 1)));
            }
            ...
            if (디버그모드)
            {
                pk::info(pk::format("[CHK-RES] ref_troops={} unit_troops={} unit_food={}", ref_troops, unit_troops, unit_food));
            }
            ...
            if (do_cmd)
            {
                ...
                cmd.weapon_amount[0] = pk::max(1, unit_troops);
                ...
                if (디버그모드)
                {
                    pk::info(pk::format("[CHK-CMD] type={} troops={} food={} gold={} tgt=({}, {})",
                        cmd.type, cmd.troops, cmd.food, cmd.gold, cmd.target_pos.x, cmd.target_pos.y));
                    pk::info(pk::format("[CHK-CMD] member[0]={} w0(id={},amt={}) w1(id={},amt={})",
                        cmd.member[0], cmd.weapon_id[0], cmd.weapon_amount[0], cmd.weapon_id[1], cmd.weapon_amount[1]));
                }
                if (디버그모드)
                {
                    string unit_name = pk::decode(pk::get_name(leader));
                    pk::info(pk::format("[PushCombatUnit] 리더 행동상태 {}: {}", unit_name, leader.action_done ? "완료" : "미완료"));
                }
                // 출진.
                int unit_id = pk::command(cmd);
                pk::unit@ unit_cmd = pk::get_unit(unit_id);
                if (pk::is_alive(unit_cmd))
                {
                    if (디버그모드)
                    {
                        ...
                        string dst_name  = pk::decode(pk::get_name(dst_base));
                        string unit_name = pk::decode(pk::get_name(leader));
                        pk::info(pk::format("[PushCombatUnit] 생성 {}대 {}→{} ID={}", unit_name, src_name, dst_name, unit_id));
                    }
                    unit_cmd.action_done = true;
                ...
                else if (디버그모드)
                {
                    ...
                    string dst_name  = pk::decode(pk::get_name(dst_base));
                    string unit_name = pk::decode(pk::get_name(leader));
                    pk::info(pk::format("[PushCombatUnit] 생성 실패 {}대 {}→{}", unit_name, src_name, dst_name));
                }
            }
        ...
        // 제일 많은 무기 선택 함수
        void get_ground_weapon(pk::building@ base, int troops_max, int &out weapon_sel, int &out troops_sel)
        {
        ...
        bool PushAttackSupport(pk::building@ dst_base, pk::force@ force)
        {
            ...
            {   // 공격측 우세
                ...
                int num_atk_support_unit = count_atk_unit_type(src_base, dst_base, 부대종류_수송);
                int unit_type = (!거점공략_수송부대허용 or pk::enemies_around(src_base) or num_atk_support_unit > max_unit_supply)? 부대종류_전투 : 부대종류_수송;
                cmd = (PushAttackUnit(dst_base, src_base, unit_type, /*max_unit*/1) or cmd); // 전투 or 수송부대
            }
            else if (0 <= status and status < (2*ref_status))
            {   // 공방 치열
                status_info = "공방 치열";
                int num_atk_support_unit = count_atk_unit_type(src_base, dst_base, 부대종류_전투);
                cmd = (PushAttackUnit(dst_base, src_base, 부대종류_전투, /*max_unit*/1) or cmd); // 전투 or 공성부대
            }
        ...
        int max_unit_combat = pk::max(1, pk::min(50, 거점공략_전투부대수));
        int max_unit_supply = pk::max(1, pk::min(1, 거점공략_수송부대수));
        ...
        // 목표 적 거점으로 진군 중인 부대수
        int count_atk_unit_type(pk::building@ src_base, pk::building@ dst_base, int check_type)
        {
            int count = 0;
            if (!pk::is_alive(src_base) or !pk::is_alive(dst_base)) return -1;
            int src_id = src_base.get_id();
